	...
	partial class EditorUtils {
		...
		public static BBParameter BBParameterField(string prefix, BBParameter bbParam, bool blackboardOnly = false, MemberInfo member = null){
			...
			if (bbParam.varRef != null && bbParam.varType != bbParam.refType){
				if (blackboardOnly){
					info = string.Format("AutoCast: ({0} ➲ {1})", bbParam.varType.FriendlyName(), bbParam.refType.FriendlyName() );
				} else {
					info = string.Format("AutoCast: ({0} ➲ {1})", bbParam.refType.FriendlyName(), bbParam.varType.FriendlyName() );
				}
			}
