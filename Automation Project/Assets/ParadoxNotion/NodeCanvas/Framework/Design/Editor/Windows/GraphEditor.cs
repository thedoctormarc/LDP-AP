...
namespace NodeCanvas.Editor{
	...
	public class GraphEditor : EditorWindow{
		...
		const string WATERMARK = "@NodeCanvas Framework v2.6.0";
		...
		void OnGUI(){
			...
			var setDirty = false;
			if ( (e.type == EventType.MouseUp && e.button != 2) || (e.type == EventType.KeyUp && (e.keyCode == KeyCode.Return || GUIUtility.keyboardControl == 0 ) ) ){
				setDirty = true;
			}
		...
		void EndZoomArea(){
			...
			zoomRecoveryRect.width = screenWidth;
			zoomRecoveryRect.height = screenHeight;
...
/*
		...
		void DoHotBoxPanel(){
			...
			if (isShowingHotBox){
				var hotboxRect = new Rect( 50, 50, screenWidth - 100, screenHeight - 100 );
