...
*/
		...
		void ShowHeader(){
			...
			if (inIconMode){ //prefs in icon mode AND has icon
				...
				if (name != null){
					...
					GUILayout.Label(string.Format("<b><size=12><color=#{0}>{1}</color></size></b>", hexColor, title), centerLabel);
				}
			}
		...
		void HandleContextMenu(Event e){
		    ...
		    var isContextClick = (e.button == 1 && e.type == EventType.MouseUp) || (e.control && e.type == EventType.MouseUp || e.type == EventType.ContextClick);
